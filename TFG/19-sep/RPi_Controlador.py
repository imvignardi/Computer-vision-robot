...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.setWindowFlags(Qt.CustomizeWindowHint | Qt.WindowTitleHint | Qt.WindowSystemMenuHint)
    ...
    def initUI(self):
        ...
        self.calibMsgBox.setIcon(QMessageBox.Icon.Question)
        self.calibMsgBox.buttonClicked.connect(self.popup_actions)
        self.calibMsgBox.setWindowFlags(Qt.CustomizeWindowHint | Qt.WindowTitleHint)
        
        self.calibMsgBox2 = QMessageBox()
        self.calibMsgBox2.setWindowModality(Qt.ApplicationModal)
        ...
        self.calibMsgBox2.setWindowFlags(Qt.CustomizeWindowHint | Qt.WindowTitleHint)
        self.calibMsgBox2.setIcon(QMessageBox.Icon.Warning)
        ...
        self.posSavedMsgBox.setWindowFlags(Qt.CustomizeWindowHint | Qt.WindowTitleHint)
        self.posSavedMsgBox.setIcon(QMessageBox.Icon.Information)
        ...
        self.focusCamera.setWindowFlags(Qt.CustomizeWindowHint | Qt.WindowTitleHint)
        self.focusCamera.setIcon(QMessageBox.Icon.Information)
        ...
        self.focusCameraOk.setWindowFlags(Qt.CustomizeWindowHint | Qt.WindowTitleHint)
        self.focusCameraOk.setIcon(QMessageBox.Icon.Question)
        ...
        self.initiating.setWindowFlags(Qt.CustomizeWindowHint | Qt.WindowTitleHint)
        self.initiating.setIcon(QMessageBox.Icon.Warning)
        ...
        self.savingPosMessage.addButton(QPushButton('Accept'), QMessageBox.YesRole)
        self.savingPosMessage.setIcon(QMessageBox.Icon.Information)
        ...
        self.erasedAll.setIcon(QMessageBox.Icon.Information)
        self.erasedAll.setWindowFlags(Qt.CustomizeWindowHint | Qt.WindowTitleHint)
        ...
        self.paused.setWindowFlags(Qt.CustomizeWindowHint | Qt.WindowTitleHint)
        self.paused.setIcon(QMessageBox.Icon.Information)

        self.resume = QMessageBox()
        self.resume.setWindowTitle("Robot movement resumed")
        self.resume.setText("The robot has resumed its movement")
        self.resume.setWindowFlags(Qt.CustomizeWindowHint | Qt.WindowTitleHint)
        self.resume.setIcon(QMessageBox.Icon.Warning)
    ...
    def assign_obj(self):
        self.myThread.stop()
        while self.myThread.returnFinish()==False or self.updating==True:
            continue
        del self.myThread
    ...
    def killWindow(self, str, int):
        myNum = int
        if myNum==1:
            self.endCalib=str
            self.calibMsgBox2.done(0)
            self.calibMsgBox.done(0)
        elif myNum==2:
            self.errorByte=str
            self.savingPosMessage.done(0)

    def popup_actions(self):
        arduino.write((15).to_bytes(1, "little")) #COMMENT FOR PROGRAM TESTING
        #Warning message
        thread = controlThread(1)
        thread.controlsignal.connect(self.killWindow)
        thread.start()
        ...
        if check == QMessageBox.Abort:
            ...
            arduino.write((255).to_bytes(1, "little"))
    ...
    def popup_actions2(self):
        ...
        while self.focused==False:
            if self.initialFocus==False:
                arduino.write((1).to_bytes(1, "little"))
                ...
                if self.focusCameraOk.standardButton(self.focusCameraOk.clickedButton()) == QMessageBox.Yes:
                    arduino.write((2).to_bytes(1, "little"))
                else:
                    arduino.write((1).to_bytes(1, "little"))
    ...
    def on_release_no_move(self):
        ...
        while arduino.in_waiting>0:
            t = arduino.read()
        arduino.write((0).to_bytes(1, "little"))
        arduino.flush()
    ...
    def on_click_save(self):
        ...
        if self.errorByte=='0x1':
            self.posSavedMsgBox.exec_()
        elif self.errorByte=='0xe3':
            self.samePointMsg.exec_()
        elif self.errorByte=='0xe4':
...
class controlThread(QThread):
    controlsignal = pyqtSignal(str, int)
    ...
    def run(self):
        ...
        t = arduino.read()
        t = hex(ord(t))
        self.controlsignal.emit(t, self.who)
